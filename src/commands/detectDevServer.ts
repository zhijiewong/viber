...
export async function detectDevServerCommand(webviewProvider: WebviewProvider): Promise<void> {
    try {
        ...
        if (detectedServers.length === 0) {
            vscode.window.showInformationMessage('No local development servers detected. Try starting your dev server first.');
            return;
        }
        ...
        vscode.window.showErrorMessage(`Failed to detect dev server: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
}
