...
export class ClipboardManager {
    public static async copyElementInfo(element: ElementInfo, format: 'json' | 'css' | 'xpath' = 'json'): Promise<void> {
        let content: string;
        ...
        vscode.window.showInformationMessage(`Element ${format.toUpperCase()} copied to clipboard`);
    }
    ...
    public static async copyMultipleSelectors(selectors: { css: string; xpath: string; }): Promise<void> {
        const content = `CSS Selector: ${selectors.css}\nXPath: ${selectors.xpath}`;
        await vscode.env.clipboard.writeText(content);
        vscode.window.showInformationMessage('Selectors copied to clipboard');
    }
