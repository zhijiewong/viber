...
export class HTMLProcessor {
    ...
    try {
      ...
      const sanitized = sanitizeHtml(preProcessed, {
        ...
        transformTags: {
          ...
          script: (tagName: string, attribs: Attributes) => {
            // Check if it's a DOM Agent script
            if (
              attribs.id === 'dom-agent-selector-styles' ||
            ...
            return {
              tagName: 'div',
              attribs: { style: 'display:none;', 'data-removed-script': 'true' },
            };
          },
  ...
  private megaCleanDocumentWrite(html: string): string {
    ...
    cleaned = cleaned.replace(
      /<script[^>]*>[\s\S]*?document\s*\.\s*write[\s\S]*?<\/script>/gi,
    ...
    cleaned = cleaned.replace(
      /href\s*=\s*["'][^"']*document\s*\.\s*write[^"']*["']/gi,
      'href="#removed"'
    );
    cleaned = cleaned.replace(
      /src\s*=\s*["'][^"']*document\s*\.\s*write[^"']*["']/gi,
      'src="#removed"'
    );
    ...
    cleaned = cleaned.replace(
      ...
      '/* document.writeln removed */'
    );

    // 🔥 Remove any lines containing 'document.write' text
    cleaned = cleaned.replace(
      /.*document\s*\.\s*write.*$/gim,
      '<!-- line containing document.write removed -->'
    );
...
${cleanedBody}
   ...
   */
  private cleanDOMNode(node: Node): string {
    if (!node) {
      return '';
    }
